...
class WebServer extends EventEmitter {
    constructor(config, settings, loggers) {
        ...
        // Session State
        this.sessionCode = crypto.randomBytes(3).toString('hex').toUpperCase(); // 6 chars
        this.adminPassword = crypto.randomBytes(4).toString('hex').toLowerCase(); // 8 chars
        ...
        this.connectionAttempts = new Map(); // IP -> { count, lockedUntil }

        this.remoteControllerSocket = null;
    }
    ...
    createHttpServer() {
        ...
        this.httpServer.listen(this.httpPort, () => {
            log.info(`Mobile app available at http://localhost:${this.httpPort}`);
        });
    }
    ...
    handleConnection(ws, req) {
        ...
        ws.on('message', async (data) => {
            try {
                ...
                if (msg.type === 'admin-auth') {
                    if (msg.sessionCode && msg.sessionCode.toUpperCase() !== this.sessionCode) {
                        this.recordAuthFailure(ws.clientIp);
                        ws.send(JSON.stringify({ type: 'admin-auth-result', success: false, error: 'session_mismatch' }));
                        ws.close(1008);
                        return;
                    }
                    if (msg.password === this.adminPassword) {
                        ...
                        try {
                            const uiHtml = fs.readFileSync(path.join(this.config.rootPath, 'mobile', 'admin-ui.html'), 'utf8');
                            const uiScript = fs.readFileSync(path.join(this.config.rootPath, 'mobile', 'admin-ui.js'), 'utf8');
                            ...
                            log.error('Failed to read admin UI files:', e);
                            // Still let them in but UI might be broken? Or better to fail or send empty?
                            // Sending partial result might be safer to avoid total lockout if one file misses
                            ws.send(JSON.stringify({
                                type: 'admin-auth-result', success: true,
                                settings: this.currentSettings, blockedIps: Array.from(this.blockedIps),
                                uiHtml: '<div>Error loading UI</div>',
                                uiScript: ''
                            }));
                        }
                    } else {
        ...
        ws.on('close', () => {
            if (this.remoteControllerSocket === ws) {
                this.emit('remote-control-end');
                this.remoteControllerSocket = null;
            }
    ...
    handleAdminMessage(ws, msg) {
        if (msg.type === 'admin-settings') {
            ...
            if (this.remoteControl && this.currentSettings.remoteEnabled) {
                ...
                else if (msg.type === 'remote-control-start') {
                    this.remoteControllerSocket = ws;
                    this.emit('remote-control-start');
                }
                else if (msg.type === 'remote-control-end') {
                    if (this.remoteControllerSocket === ws) this.remoteControllerSocket = null;
                    this.emit('remote-control-end');
                }
            }
        ...
        } else if (msg.type === 'admin-toggle-mode') {
    ...
    getLocalIP() {
        ...
        candidates.sort((a, b) => {
            const getScore = (c) => {
                ...
                if (lowerName.includes('wi-fi')) return 100;
                if (lowerName.includes('ethernet') && !lowerName.includes('vethernet')) return 80;
                if (lowerName.includes('wlan')) return 60;
                if (lowerName.includes('eth')) return 50;
                return 10;
            };
...
<body>
  <div class="container">
    ...
    <div class="form-group">
      ...
      <input type="text" id="code-input" placeholder="ABC123" autocomplete="off" autocapitalize="characters" autofocus>
    </div>
  ...
  <script>
    ...
    input.addEventListener('input', () => { input.value = input.value.toUpperCase(); error.classList.remove('visible'); });
    function join() {
      const code = input.value.trim();
      if (!code) { error.textContent = 'Please enter a session code'; error.classList.add('visible'); return; }
...
<a href="/">Try Again</a>
    ...
    recordAuthFailure(ip) {
        if (!ip) return;
        const entry = this.connectionAttempts.get(ip) || { count: 0, lockedUntil: 0 };
        // Reset count if lock expired long ago (e.g. 10 mins)
        if (entry.lockedUntil < Date.now() - 600000) entry.count = 0;

        entry.count++;
        // Block after 10 failed attempts for 5 minutes
        if (entry.count >= 10) {
            entry.lockedUntil = Date.now() + 300000; // 5 mins
            log.info(`[Security] IP ${ip} blocked due to excessive auth failures.`);
        }
        this.connectionAttempts.set(ip, entry);
    }
}
